...
def main():
    ...
    # counts_df = counts_df[~counts_df['locus'].str.split(':').str[0].isin({'chrX', 'chrY'})]  # Autosomes only
    counts_df['ens_id'] = counts_df['gene_id'].str.split('.').str[0]
    counts_df = pd.DataFrame(counts_df.groupby('ens_id')['FPKM'].sum(), columns=['FPKM'])
