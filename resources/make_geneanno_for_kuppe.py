...
def main():
    ...
    counts_df = pd.read_csv(args.counts_file, sep='\t', header=0)
    counts_df = counts_df[['ENSEMBL.ID', 'Raw.Counts']]

    df_merged = geneanno.merge(counts_df, how='left', left_index=True, right_on='ENSEMBL.ID', validate='1:1')
    df_out = df_merged.loc[:, 'Raw.Counts']
